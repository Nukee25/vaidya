...
class DiagnosisReportView(APIView):
        ...
        if isinstance(raw_predictions, list):
            for item in raw_predictions:
                ...
                if len(normalized) == 3:
                    break
        if len(normalized) < 3:
            defaults = DiagnosisReportView._build_ollama_prompt([])["predicted_diseases"]
    ...
    def _build_report_from_ollama(symptom_cards, gender=None, age=None):
        valid_cards = DiagnosisReportView._normalized_symptom_cards(symptom_cards)
...
class LoginView(APIView):
    ...
    def post(self, request):
        ...
        return Response({"message": "Login successful", "username": user.username, "status": "success"}, status=status.HTTP_200_OK)
...
class PredictView(APIView):
    ...
    def post(self, request):
        ...
        try:
            report_payload = DiagnosisReportView._build_report_from_ollama(symptom_cards, gender=gender, age=age)
...
class HealthScoreView(APIView):
    ...
    def get(self, request):
        ...
        return Response({"score": DiagnosisReportView._calculate_health_score(reports)}, status=status.HTTP_200_OK)
